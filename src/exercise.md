...
ある地域の水道料金は、使用水量に応じて次のように計算されます。

- 使用水量が \\(0 \rm{m}^3\\) 以上 \\(10 \rm{m}^3\\) 以下の場合 : 基本料金 1200 円
- 使用水量が \\(10 \rm{m}^3\\) を超え \\(20 \rm{m}^3\\) 以下の場合 : 基本料金 1200 円に加えて \\(10 \rm{m}^3\\) を超えた分について \\(1 \rm{m}^3 \\) あたり 120 円
- 使用水量が \\(20 \rm{m}^3\\) を超えた場合 : 基本料金 1200 円に加えて、 \\(10 \rm{m}^3\\) から \\(20 \rm{m}^3\\) までの\\(10 \rm{m}^3 \\) 分は \\(1 \rm{m}^3 \\) あたり 120 円、\\(20 \rm{m}^3\\) を超えた分については \\(1 \rm{m}^3 \\) あたり 180 円

使用水量 (\\(\rm{m}^3\\) を実数値で入力すると水道料金を計算して出力するプログラムを作成してください。

入力される使用水量は 0 以上 100 以下の実数値とします。(入力値のチェックは必要ありません)

出力される水道料金については、小数点以下は切り捨てとします。

実行例を示します。
1 行目は入力された使用水量、2 行目は出力された水道料金です。
...
自然数 \\(n\\) について、\\(n\\) 自身を除く正の約数の和が \\(n\\) と等しくなるとき、\\(n\\) を完全数と呼びます。

例えば \\(6\\) の正の約数は \\(1, 2, 3, 6\\) です。このうち \\(6\\) 自身を除いた約数の和は
\\[
1 + 2 + 3 = 6  
\\]
となるので、\\(6\\) は完全数です。

正の整数 \\(m\\) を入力すると \\(1\\) 以上 \\(m\\) 以下の完全数をすべて求めて表示するプログラムを作成してください。

入力される値は \\(1\\) 以上 \\(100000\\) 以下の整数値とします。(入力値のチェックは必要ありません)
出力では、まず `perfect numbers:` と表示し、その次の行から完全数を 1 行に 1 つづ表示してください。

実行例を示します。
1 行目が入力された正の整数で、2 行目以降がプログラムの出力です。
