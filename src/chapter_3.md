...
以下に示すプログラムは、
初項 \\( a_1 = 1 \\)、漸化式 \\( a_{n+1} = 2 a_n + 1\ (1 \le n) \\) で定まる数列 \\(a_n\\) を
...
演習 3-2 を参考に、初項 \\( x_1 = 0.2\\)、漸化式 \\( x_{n+1} = 4 x_n (1 - x_n) \ (1 \le n) \\) で定まる数列 \\(x_n\\) を第 10 項まで求めて表示するプログラムを作成し、動作結果を確認してください。
なお、\\(x_n\\) は一般に実数となるので、対応するデータは `double` 型を用いて計算するようにしてください。

また、`double` 型の変数 `x` の値を表示するときは、以下のように `printf` を使うとよいです。
```c:
  printf("%f\n", x)
```
