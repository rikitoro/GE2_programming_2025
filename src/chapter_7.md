...
演習 6-2 では、プログラムを実行すると、"Enter your order number: Coffee (101), Tea (202) or Milk (303)" と表示し、
...
ただし、ユーザーの入力値が 0 の場合は、Water と表示したうえで繰り返しを終了してください。
...
ユーザーの入力が 0 以外の場合は、数値に対応する飲み物の名前が表示され、再度オーダーの入力を促すメッセージが表示されます。
0 を入力した場合は Water と表示され、プログラムが終了します。
